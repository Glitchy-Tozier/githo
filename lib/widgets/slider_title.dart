...
class SliderTitle extends StatelessWidget {
  ...
  /// Specify how the sub-elements should look by using
  /// ['normal', …] or ['bold', …].
  ...
  Widget build(BuildContext context) {
    final List<TextSpan> textSpanList = <TextSpan>[];

    // Style the spans according to the input
    for (final List<String> textDataPair in textDataPairs) {
      final String style = textDataPair[0];
      final String text = textDataPair[1];
      if (style == 'normal') {
        textSpanList.add(
          TextSpan(
            text: text,
